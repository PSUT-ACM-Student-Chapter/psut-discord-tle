...
from tle.util import events, paginator, tasks
...
class ProblemCache:
    ...
    async def _update(self, problems):
        ...
        for problem in self.problems:
            problem_contest = self.cache_master.contest_cache.contest_by_id.get(
                problem.contestId
            )

            divisions = [
                div_tag for div_tag in _DIV_TAGS if problem_contest.matches([div_tag])
            ]
...
class ProblemsetCache:
    ...
    async def _fetch_problemsets(self, contests, *, force_fetch=False):
        ...
        # different times.
        contests = [c for c in contests if not getattr(c, "is_atcoder", False)]
...
class RatingChangesCache:
    ...
    async def _monitor_task(self, _):
        ...
        contest_changes_pairs = await self._fetch(
            self.monitored_contests, auto_save=False
        )
