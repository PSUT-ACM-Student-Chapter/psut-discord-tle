...
class Contests(commands.Cog):
    ...
    @commands.hybrid_group(
        description="Commands for listing contests", invoke_without_command=True
    )
    ...
    @commands.hybrid_group(
        description="Commands for contest reminders", invoke_without_command=True
    )
    ...
    def _filter_rated_only_contestant_data(handles, ranklist):
        ...
        for contestant in ranklist.standings:
            ...
            if handle in ranklist.delta_by_handle:
                ...
                dict["rank"] = (
                    current_rank if current_score != last_score else last_rank
                )
                last_rank = dict["rank"]
                last_score = current_score
                ranklist.standing_by_id[handle] = cf.make_from_dict(
                    cf.RanklistRow, dict
                )
    ...
    @commands.hybrid_command(
        description="Show ranklist for given handles and/or server members"
    )
    ...
    @commands.hybrid_command(
        description="Start a rated vc.", usage="<contest_id> <@user1 @user2 ...>"
    )
    ...
    @commands.hybrid_command(
        description="Unregister this user from an ongoing ratedvc", usage="@user"
    )
    ...
    @commands.hybrid_command(
        description="Set the rated vc channel to the current channel"
    )
    @commands.has_role(constants.TLE_ADMIN)
    ...
    @commands.hybrid_command(
        description="Plot vc rating for a list of at most 5 users",
        usage="@user1 @user2 ..",
    )
    async def vcrating(self, ctx, *members: discord.Member):
        ...
        labels = [
            gc_graph.StrWrap("{} ({})".format(member_display_name, rating_data[-1][1]))
    ...
    @commands.hybrid_command(
        description="Plot vc performance for a list of at most 5 users",
        aliases=["vcperf"],
        usage="@user1 @user2 ..",
    )
    async def vcperformance(self, ctx, *members: discord.Member):
        ...
        plt.ylim(min_rating - 100, max_rating + 200)
        labels = [
            gc_graph.StrWrap("{} ({})".format(member_display_name, ratingbefore))
            for member_display_name, rating_data in plot_data.items()
        ]
        plt.legend(labels, loc="upper left", prop=gc_graph.fontprop)
    ...
    @commands.hybrid_command(
        description="Estimation of contest problem ratings",
        aliases=["probrat"],
        usage="contest_id",
    )
