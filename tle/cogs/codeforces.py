...
from tle.util import discord_common, paginator
...
class Codeforces(commands.Cog):
    ...
    async def _gitgud(self, ctx, handle, problem, delta, hidden):
        ...
        monthlyPointsStr = (
            monthlypoints if not hidden else "||" + str(monthlypoints) + "||"
        )
    ...
    async def upsolve(self, ctx, choice: int = -1):
        ...
        if choice > 0 and choice <= len(problems):
            ...
            def make_page(chunk, pi, num):
                ...
                msg = "\n".join(
                    make_line(10 * pi + i, prob) for i, prob in enumerate(chunk)
                )
    ...
    @commands.hybrid_command(
        description="Recommend a problem",
    ...
    async def gimme(self, ctx, *args):
        ...
        problems = [
            prob
            for prob in cf_common.cache2.problem_cache.problems
            if prob.rating >= srating
            and prob.rating <= erating
        ...
        embed = discord.Embed(title=title, url=problem.url, description=desc)
        ratingStr = (
            problem.rating if srating == erating else "||" + str(problem.rating) + "||"
        )
        embed.add_field(name="Rating", value=ratingStr)
        if tags:
    ...
    @commands.hybrid_command(
        description="List solved problems",
    ...
    async def stalk(self, ctx, *args):
        ...
        handlesWithUrl = [
            "`{}` (https://codeforces.com/profile/{})".format(handle, handle)
        ...
        def make_page(chunk):
            ...
            title = "{} solved problems by {}".format(
                "Hardest" if hardest else "Recently", ", ".join(handlesWithUrl)
            )
    ...
    async def mashup(self, ctx, *args):
        ...
        problems = sorted(
            [problems[k] for k in choices], key=lambda problem: problem.rating
        )
        msg = "\n".join(
    ...
    async def gitgud(self, ctx, *args):
        ...
        - Get the latest problemm with --latest
    ...
    async def gitlog(self, ctx, member: discord.Member = None):
        ...
        def make_page(chunk, score):
            message = discord.utils.escape_mentions(
                f"Gitgud log for {member.display_name} (total score: {score})"
            )
    ...
    async def nogudlog(self, ctx, member: discord.Member = None):
        ...
        def make_page(chunk):
            message = discord.utils.escape_mentions(
                f"Nogud log for {member.display_name}"
            )
            log_str = "\n".join(make_line(entry) for entry in chunk)
            embed = discord_common.cf_color_embed(description=log_str)
            return message, embed
    ...
    @commands.hybrid_command(
        description="Report challenge completion", aliases=["gotbad"]
    )
    @cf_common.user_guard(group="gitgud")
    async def gotgud(self, ctx):
        ...
        rc = cf_common.user_db.complete_challenge(
            user_id, challenge_id, finish_time, score
        )
    ...
    async def _nogud(self, ctx, member: discord.Member):
        ...
        if not active:
            await ctx.send(
                f"No active challenge found for user `{member.display_name}`."
            )
            return
    ...
    @commands.hybrid_command(
        description="Recommend a contest", usage="[handles...] [+pattern...]"
    )
    async def vc(self, ctx, *args: str):
        ...
        if not markers:
            ...
            markers = (
                ["div3"]
                if divr < 1600
                else ["div2"] if divr < 2100 else div1_indicators
            )
        ...
        recommendations.sort(
            key=lambda contest: cf_common.cache2.contest_cache.get_contest(
                contest
            ).startTimeSeconds,
            reverse=True,
        )
    ...
    @commands.hybrid_command(
        description="Display unsolved rounds closest to completion", usage="[keywords]"
    )
    ...
    @commands.hybrid_command(
        description="Calculate team rating", usage="[handles] [+peak]"
    )
